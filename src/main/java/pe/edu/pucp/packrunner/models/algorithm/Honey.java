...
public class Honey implements Comparable<Honey> {
     ...
     * @param: size of n
     */
    public Honey(int size, ArrayList<Double> personalBest, double[] factors) {
        ...
        for(int i=0;i<nectar.length;i++){
            ...
            this.nectar[i]=personalBest.get(i);
        }
        //initNectar();
    }
    ...
    public void computeConflicts() { // compute the number of conflicts to calculate fitness
        ...
        boolean done = false; // used to check is checking for diagonal is out of bounds
        ...
        for (int i = 0; i < MAX_LENGTH; i++) {
            x = i;
            y = (int) this.nectar[i]; // will result to no horizontal and vertical conflicts because it will result to
                                      // diagonal
